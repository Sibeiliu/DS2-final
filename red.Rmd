...
rpart.fit <- train(quality~.,red, 
                   method = "rpart",
                   tuneGrid = data.frame(cp = exp(seq(-9,-1, length = 20))),
                   metric="ROC",
                   trControl = ctrl)
ggplot(rpart.fit, highlight = TRUE) + theme_bw()

rpart.fit$bestTune[,1]

set.seed(1)
tree1 = rpart(formula =quality ~., data=red,cp=rpart.fit$bestTune[,1])
